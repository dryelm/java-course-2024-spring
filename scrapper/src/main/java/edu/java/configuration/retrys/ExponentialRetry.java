...
public class ExponentialRetry extends Retry {
    ...
    public ExponentialRetry(int maxAttempts, Duration minDelay, int appender) {
    ...
    public Mono<Long> getRetry(Retry.RetrySignal rs) {
        ...
        if (rs.totalRetries() < maxAttempts) {
            ...
            Duration delay = minDelay.plusSeconds((long) Math.pow(appender.getSeconds(), rs.totalRetries()));
